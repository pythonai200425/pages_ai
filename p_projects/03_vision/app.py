...
if chosen is not None:
    ...
    with col1:
        if st.button("🧠 Predict cat vs dog"):
            try:
                x = preprocess_for_model(chosen)
                pred = model.predict(x, verbose=0)[0][0]  # sigmoid output
                st.write(f"Model output (sigmoid): **{pred:.4f}**")

                # By your code: >0.5 => Dog, else Cat
                label = "Dog 🐶" if pred > THRESHOLD else "Cat 🐱"
                st.success(f"Prediction: **{label}**")

                # Optional: show what the model *thinks* as probabilities
                st.info(f"Dog probability: {pred:.2%} | Cat probability: {(1-pred):.2%}")
